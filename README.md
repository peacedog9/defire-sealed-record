...
Before any plaintext reveal, the same runner validates the entire public issuance sequence and hash chain. For the newest hash-only receipt it independently verifies the canonical commitment, pinned DigiCert root, RFC 3161 signature and signed-time delay, and the matching OpenTimestamps proof. A new production signal therefore receives an outside verification receipt as soon as its public hash appears.
